...
function processArgs() {
  ...
  if (command === 'create-page') {
    ...
    workdir = currentWorkDir;
    distdir = path.join(workdir, '..', 'dist');
  } else if (command === 'image-performance') {
    workdir = currentWorkDir;
    distdir = path.join(workdir, '..', 'dist');
  } else {
    how2use();
...
/**
 * @param {Array} dir - Array of directories to create
 */
 ...
 */
async function executeCommand(cmd, _params) {
...
const fn = { 'create-page': createPage, 'build': build, 'build-page': buildPage, 'create-wc': createWc, 'scafolding': createScafolding, 'pwa': pwa, 'env-vars': envVars, 'image-performance': imagePerformance };
